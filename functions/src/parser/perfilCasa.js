...
const REFEICOES_VALIDAS = ["café da manhã", "almoço", "jantar"];
const REFEICOES_PADRAO = REFEICOES_VALIDAS;

// Formato aceito, uma linha por pessoa (Nome: item1; item2; ...), mais três
// linhas especiais (Observações:/Horário:/Refeições:) em qualquer posição do
// texto:
//
...
export function parsearPerfilCasa(texto, perfilAnterior = null) {
  ...
  let refeicoes = perfilAnterior?.refeicoes || REFEICOES_PADRAO;
  const erros = [];
  ...
  for (const linha of linhas) {
    ...
    if (/^refei[cç][oõ]es$/i.test(chave)) {
      const escolhidas = valor
        .split(";")
        .map((r) => r.trim().toLowerCase())
        .filter(Boolean);
      const invalidas = escolhidas.filter((r) => !REFEICOES_VALIDAS.includes(r));

      if (invalidas.length || !escolhidas.length) {
        erros.push(
          `Refeições "${valor}" inválidas — mantendo a configuração anterior (${refeicoes.join(", ")}). Use: ${REFEICOES_VALIDAS.join(", ")}.`
        );
      } else {
  ...
  return { pessoas, observacoesGerais, horarioEnvio, refeicoes, erros };
}
...
export function formatarPerfilCasa(perfil) {
  ...
  return [
    ...
    `Refeições: ${(perfil.refeicoes || REFEICOES_PADRAO).join(", ")}`,
  ]
